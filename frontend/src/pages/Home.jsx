...
const Home = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5555/books')
      .then((response) => {
        setBooks(response.data.data); //why is data.data done?
        setLoading(false);
      })
      .catch((error) => {
        console.log(error);
  ...
  return (
    <div className='p-4'>
      ...
      */}
      ...
      {loading ? ( 
      //This basically says that if loading from the useState is true then use Spinner & 
      //if not then use the other block of the ternary operator
